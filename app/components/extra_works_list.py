    ...
    def refresh_table() -> None:
        def load() -> None:
            with get_db() as db:
                ...
                works = get_extra_works(db, object_id=selected_object)
                total = len(works) if object_id is not None else len(get_extra_works(db, object_id=None))
                rows = [{
                    'id': work.id,
